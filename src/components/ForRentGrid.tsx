...
// Rental-specific status labels
const RENT_TABS = ['Available', 'Leased', 'All'] as const;
...
// Rentals now arrive server-rendered via props (SSR) — same markup as before.
export default function ForRentGrid({ initialListings, initialError }: { initialListings: any[]; initialError?: string | null }) {
  ...
  const [tab,        setTab]        = useState<RentTab>('Available');
  ...
  filtered = [...filtered].sort((a, b) =>
    ...
    sort === 'dom_asc'    ? (a.days_on_market || 999) - (b.days_on_market || 999) :
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: BODY }}>
      ...
      <div style={{ position:'relative', height:isMobile?380:480, overflow:'hidden', marginTop:'-72px' }}>
        ...
        <img src="https://images.mlrecloud.com/site/el-cid-homes/el-cid-historic-homes-west-palm-beach.jpg" alt="El Cid homes for rent in West Palm Beach, Florida" style={{ width:'100%', height:'100%', objectFit:'cover', objectPosition:'center 35%' }} />
        ...
        <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'flex-end', padding:isMobile?'0 24px 40px':'0 40px 60px', textAlign:'center' }}>
          ...
          <p style={{ color:'rgba(255,255,255,0.7)', fontSize:isMobile?13:16, maxWidth:520, margin:0, fontFamily:BODY, lineHeight:1.65 }}>
            Homes for rent in El Cid, West Palm Beach · Updated daily from BeachesMLS
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
        ...
        {/* Filter bar */}
        <div style={{ background: '#fff', borderRadius: 16, padding: '20px 24px', margin: '24px 0', border: '1px solid #e2e8f0', display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {RENT_TABS.map(t => (
              <button key={t} onClick={() => setTab(t)}
                style={{ padding: '8px 18px', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: DISPLAY, fontSize: 13, fontWeight: 600, background: tab === t ? TEAL : '#f1f5f9', color: tab === t ? '#fff' : SLATE, transition: 'all 0.15s' }}>
                {t} ({counts[t]})
              </button>
            ))}
          </div>
        ...
        <div style={{ marginBottom: 16, fontSize: 13, color: err ? '#dc2626' : SLATE, fontFamily: BODY }}>
